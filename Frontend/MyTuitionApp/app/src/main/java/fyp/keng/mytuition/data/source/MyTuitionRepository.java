...
public class MyTuitionRepository implements MyTuitionDataSource {

    private static MyTuitionRepository instance = null;

    private final MyTuitionRemoteDataSource remote;

    private final MyTuitionLocalDataSource local;
    ...
    public MyTuitionRepository(MyTuitionRemoteDataSource remote, MyTuitionLocalDataSource local) {
        this.remote = remote;
        this.local = local;
    }

    public static MyTuitionRepository getInstance(MyTuitionRemoteDataSource remote,
                                                  MyTuitionLocalDataSource local) {
        if (instance == null) {
            instance = new MyTuitionRepository(remote, local);
        }
        return instance;
    }
        ...
        this.remote.changeAvatar(body, new Callback<APIResponse<User>>() {
            ...
            public void onResponse(Call<APIResponse<User>> call, Response<APIResponse<User>> response) {
                ...
                if (resp != null && resp.isSuccessful()) {
                    ...
                    try {
                        loggedInUser.setAvatar(updateUserRequest.getUser().getAvatar().toString());
